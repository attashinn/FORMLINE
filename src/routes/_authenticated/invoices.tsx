...
import { Plus, X, Pencil, Trash2, CheckCircle, CreditCard, Loader2, Mail, Calendar } from "@/components/heroicons";
...
export const Route = createFileRoute("/_authenticated/invoices")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Manage billing, track paid revenue, and follow up on outstanding invoices." },
    ],
  }),
...
function InvoicesPage() {
    ...
    onSuccess: () => {
      ...
      toast.success(
        deliveryMethod === "scheduled"
          ...
          ? (status === "Paid" ? "Invoice created & receipt emailed" : "Invoice created & sent to client")
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (lineTotal <= 0) { toast.error(`Invoice total must be greater than ${currencySymbol}0`); return; }
    if (deliveryMethod === "scheduled" && !sendAt) { toast.error("Please specify a date for scheduled delivery"); return; }
    ...
    <div className="relative min-h-screen overflow-hidden bg-background before:pointer-events-none before:absolute before:inset-x-0 before:top-0 before:h-[520px] before:bg-[radial-gradient(ellipse_80%_60%_at_50%_-10%,rgba(124,92,255,0.2),transparent_60%)] before:content-['']">
      <main className="relative mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8 lg:py-16">
        ...
        <section className="mb-8 sm:mb-12 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="space-y-3">
            ...
            <p className="max-w-[58ch] text-muted-foreground text-sm">
              Create invoices with line items, send them directly to clients, and track your revenue.
            </p>
          </div>
          <button
        ...
        <section className="mb-8 sm:mb-10 grid grid-cols-2 divide-hairline rounded-2xl bg-surface ring-1 ring-hairline md:grid-cols-4 divide-x [&>*:nth-child(n+3)]:border-t [&>*:nth-child(n+3)]:border-hairline md:[&>*:nth-child(n+3)]:border-t-0">
          {[
            { label: "Total Billed", value: `${currencySymbol}${stats.total.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` },
            { label: "Paid Revenue", value: `${currencySymbol}${stats.paid.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, color: "text-emerald-500" },
            { label: "Unpaid (Outstanding)", value: `${currencySymbol}${stats.unpaid.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, color: "text-amber-500" },
            { label: "Overdue", value: `${currencySymbol}${stats.overdue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, color: "text-destructive" },
          ].map((s) => (
            <div key={s.label} className="px-4 py-4 sm:px-6 sm:py-5">
              ...
              <div className={`mt-2 font-serif text-2xl sm:text-3xl leading-none ${s.color || ""}`}>{s.value}</div>
            </div>
          ))}
        </section>
          ...
          {["all", "Paid", "Unpaid", "Overdue"].map((statusOption) => (
            <button
              ...
              className={`rounded-full px-4 py-1.5 text-xs font-medium ring-1 transition-all capitalize ${
                (statusOption === "all" ? filterStatus === "all" : filterStatus.toLowerCase() === statusOption.toLowerCase())
                    ...
                    <tr key={inv.id} className="hover:bg-surface-muted/30 transition-colors">
                      <td className="px-6 py-4 font-medium">
                        ...
                        {inv.lineItems.length > 0 && (
                          <div className="text-[10px] text-muted-foreground mt-0.5">{inv.lineItems.length} line item{inv.lineItems.length !== 1 ? "s" : ""}</div>
                        )}
                      </td>
                      <td className="px-6 py-4 text-muted-foreground">
                        <div>{inv.clientCompany || "—"}</div>
                        {inv.clientEmail && <div className="text-[10px] mt-0.5 text-muted-foreground/70">{inv.clientEmail}</div>}
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="inline-flex items-center gap-1">
                          ...
                          {!inv.sentAt && (
                            <button
                              onClick={() => {
                                if (confirm(`Send this invoice to ${inv.clientEmail || "the client"}?`)) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
                  ...
                  <div className="md:col-span-3 space-y-5">
                    ...
                    <div className="space-y-1.5">
                      <label className="block text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Client</label>
                      <Select
                        value={clientId}
                          ...
                          return (
                            <div key={idx} className="grid grid-cols-[1fr_75px_105px_90px_28px] gap-2 items-center">
                              ...
                              <span className="text-sm font-medium text-foreground text-right pr-2">
                                {currencySymbol}{rowTotal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                              </span>
                              <button
                                type="button"
                  ...
                  <div className="md:col-span-2 md:border-l md:border-hairline md:pl-6 space-y-5">
                    ...
                    <div className="space-y-1.5">
                      ...
                      >
                        ...
                        >
                          <SelectItem value="Unpaid" className="cursor-pointer rounded-lg py-2 pl-2.5 pr-8 focus:bg-secondary text-sm text-foreground">Unpaid</SelectItem>
                          <SelectItem value="Paid" className="cursor-pointer rounded-lg py-2 pl-2.5 pr-8 focus:bg-secondary text-sm text-foreground">Paid</SelectItem>
                          <SelectItem value="Overdue" className="cursor-pointer rounded-lg py-2 pl-2.5 pr-8 focus:bg-secondary text-sm text-foreground">Overdue</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
