...
export const Route = createFileRoute("/_authenticated/clients/")({
  head: () => ({
    meta: [
      { title: "Clients — Client Profile Hub" },
      {
        name: "description",
        ...
        >
          <div className="max-w-2xl space-y-3">
            ...
            <h1 className="font-serif text-3xl leading-[1.05] text-balance sm:text-4xl md:text-5xl lg:text-6xl">
              Client <span className="italic text-muted-foreground">profiles</span>
            </h1>
              ...
              <table className="w-full min-w-[640px] text-left text-sm">
                <thead>
                  <tr className="border-b border-hairline bg-surface-muted text-[11px] uppercase tracking-[0.14em] text-muted-foreground">
                    ...
                    <th className="px-4 sm:px-6 py-3 font-medium">Industry</th>
                    <th className="px-4 sm:px-6 py-3 font-medium">Status</th>
                    <th className="px-4 sm:px-6 py-3 font-medium">Budget</th>
                  ...
                  {filtered.map((c) => (
                    <tr key={c.id} className="group transition-colors hover:bg-secondary/60">
                      <td className="px-4 sm:px-6 py-4">
                        ...
                        >
                          <div
                            className="grid size-9 place-items-center rounded-full ring-1 ring-hairline shrink-0"
                          ...
                          <div className="min-w-0">
                            <div className="font-medium text-foreground truncate">{c.company}</div>
                            <div className="text-xs text-muted-foreground truncate">{c.fullName}</div>
                          </div>
                        </Link>
                      </td>
                      <td className="px-4 sm:px-6 py-4 text-muted-foreground whitespace-nowrap">{c.industry}</td>
                      <td className="px-4 sm:px-6 py-4 whitespace-nowrap">{statusChip(c.status)}</td>
                      <td className="px-4 sm:px-6 py-4 text-muted-foreground whitespace-nowrap">{c.budget || "—"}</td>
